...
void tst_QtJson::testNumbers_2()
{
    ...
    for (int power = 0; power <= 1075; power++) {
        ...
        // QVERIFY2(floatValues[power] == floatValues_1[power], QString("floatValues[%1] %2 != floatValues_1[%1] %3")
        //     .arg(power).arg(floatValues[power]).arg(floatValues_1[power]).toLatin1());
    }
...
{
    QJsonObject data1;
    data1.insert(QString("1"), 123.);
    QVERIFY(data1.contains(QString("1")));
    QCOMPARE(data1.value(QString("1")).toDouble(), (double)123);
    data1.insert(QString("12"), 133.);
    QCOMPARE(data1.value(QString("12")).toDouble(), (double)133);
    QVERIFY(data1.contains(QString("12")));
    data1.insert(QString("123"), 323.);
    QCOMPARE(data1.value(QString("12")).toDouble(), (double)133);
    QVERIFY(data1.contains(QString("123")));
    QCOMPARE(data1.value(QString("123")).type(), QJsonValue::Double);
    QCOMPARE(data1.value(QString("123")).toDouble(), (double)323);
}
...
void tst_QtJson::compactArray()
{
    ...
    qDebug() << "compactArray 0" << array;
    array.removeAt(1);
    qDebug() << "compactArray 0a" << array;
    doc.setArray(array);
    qDebug() << "compactArray 0b" << array;
    ...
    qDebug() << "compactArray 1" << array;
    array.removeAt(0);
    qDebug() << "compactArray 2" << array;
    doc.setArray(array);
    QVERIFY(s > doc.toBinaryData().size());
    s = doc.toBinaryData().size();
    QCOMPARE(doc.toJson(),
...
void tst_QtJson::testDetachBug()
{
    ...
    dynamic.insert(QString("Bogus"), QString("bogusValue"));
...
void tst_QtJson::testJsonValueRefDefault()
{
    ...
    QCOMPARE(empty["n/a"].toString("default"), QString("default"));
...
void tst_QtJson::arrayInitializerList()
{
    ...
    QSKIP("initializer_list is enabled only with c++11 support", SkipAll);
#else
